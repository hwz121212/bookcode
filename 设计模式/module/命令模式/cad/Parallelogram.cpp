...
void CParallelogram::OnLButtonDown( UINT nFlags, CPoint point )
{
	Point pt;
	pt.X = point.x;
	pt.Y = point.y;
...
void CParallelogram::OnLButtonUp( UINT nFlags, CPoint point )
{
	Point pt;
	pt.X = point.x;
	pt.Y = point.y;
...
void CParallelogram::OnMouseMove( UINT nFlags, CPoint point )
{
	Point pt;
	pt.X = point.x;
	pt.Y = point.y;
...
void CParallelogram::SetStartPoint( Point point )
{
	m_ptEnd = m_ptStart = point;
}

void CParallelogram::SetEndPoint( Point point )
{	
	m_ptEnd = point;
}

void CParallelogram::SetStartAndEndPoint( Point ptStart, Point ptEnd )
{
	m_ptStart = ptStart;
	m_ptEnd = ptEnd;
}

Rect CParallelogram::GetRectBound()
{
	Rect rc;
	if (m_ptStart.X > m_ptEnd.X)
	{
...
Rect CParallelogram::GetRoateRect()
{
	Rect rc;
	rc = GetRectBound();
	Rect rcRoate;
...
Rect CParallelogram::GetScaleRect()
{
	Rect rc(0, 0, 0, 0);
	return rc;
}

void CParallelogram::DrawSelected( Graphics* graphics )
{
