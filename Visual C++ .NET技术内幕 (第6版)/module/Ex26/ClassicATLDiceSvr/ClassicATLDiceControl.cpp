// ClassicATLDiceControl.cpp : Implementation of CClassicATLDiceControl
#include "stdafx.h"
#include "ClassicATLDiceControl.h"
...
STDMETHODIMP CClassicATLDiceControl::put_DiceColor(SHORT newVal)
{
	m_nDiceColor = newVal;
	LoadBitmaps();
	FireViewChange();
...
STDMETHODIMP CClassicATLDiceControl::RollDice(void)
{
	if(::IsWindow(m_hWnd)) {
	    SetTimer(1, 250);
	}
...
LRESULT CClassicATLDiceControl::OnTimer(UINT_PTR uMsg, WPARAM wParam, 
...
LRESULT CClassicATLDiceControl::OnLButtonDblClk(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    RollDice();
    return 0;
}
