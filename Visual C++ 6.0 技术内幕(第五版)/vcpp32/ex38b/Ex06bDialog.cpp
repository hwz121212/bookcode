...
BOOL CEx06bDialog::OnInitDialog() 
{
	...
	strText2.Format(_T("%3.1f"), dValue[pSlide2->GetPos()]);
	...
	LPCTSTR color[] = {_T("white"), _T("black"), _T("red"),
	                        _T("blue"), _T("yellow"), _T("cyan"),
	                        _T("purple"), _T("green")};
	...
	// top level
	tvinsert.item.pszText = _T("Homer");
	tvinsert.item.iImage = 2;
	HTREEITEM hDad = pTree ->InsertItem(&tvinsert);
	tvinsert.item.pszText = _T("Marge");
	HTREEITEM hMom = pTree->InsertItem(&tvinsert);
	// second level
	tvinsert.hParent = hDad;
	tvinsert.item.pszText = _T("Bart");
	tvinsert.item.iImage = 3;
	pTree->InsertItem(&tvinsert);
	tvinsert.item.pszText = _T("Lisa");
	pTree->InsertItem(&tvinsert);
	// second level
	...
	tvinsert.item.pszText = _T("Lisa");
	pTree->InsertItem(&tvinsert);
	tvinsert.item.pszText = _T("Dilbert");
	HTREEITEM hOther = pTree->InsertItem(&tvinsert);
	// third level
	tvinsert.hParent = hOther;
	tvinsert.item.pszText = _T("Dogbert");
	tvinsert.item.iImage = 7;
	pTree->InsertItem(&tvinsert);
	tvinsert.item.pszText = _T("Ratbert");
	pTree->InsertItem(&tvinsert);
...
void CEx06bDialog::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	case IDC_TRACKBAR1:
		strText.Format(_T("%d"), pSlide->GetPos());
		SetDlgItemText(IDC_STATIC_TRACK1, strText);
		break;
	case IDC_TRACKBAR2:
		strText.Format(_T("%3.1f"), dValue[pSlide->GetPos()]);
...
//#ifdef LATER
	...
	//	CString text;
		...
		CString string(text);
		SetDlgItemText(IDC_STATIC_TREEVIEW1, string);
	}
//#endif
	*pResult = 0;
}
